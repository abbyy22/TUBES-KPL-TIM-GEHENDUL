...
const { validateRegister, validateOrderInput } = require("../src/validators");
const { STATES, canTransition } = require("../src/utils/orderStateMachine");
...
assert.equal(
  registered.role,
  "user",
  `expected role 'user', got '${registered.role}'`,
);
...
const {
  ...
  postcondition,
  invariant,
} = require("../src/utils/contract");
assert.doesNotThrow(() => precondition(true));
assert.throws(
  () => precondition(false, "precondition test"),
  /precondition test/,
);
assert.throws(
  () => postcondition(false, "postcondition test"),
  /postcondition test/,
);
assert.throws(() => invariant(false, "invariant test"), /invariant test/);
